...
function Certificates({ bufferMode = false }) {
  ...
  const chooseOutput = (renderMode) => { setOutputChoiceOpen(false); setBatch({ ids: [...selectedIds], index: 0, renderMode }); };
  ...
  const prepare = async () => {
    ...
    try { const { blob, filename } = await downloadCertificates([currentStudent._id], endpoint, batch.renderMode); setPreview({ url: URL.createObjectURL(blob), filename }); }
  ...
  return <main className="admin-console admin-shell">
    ...
    {currentStudent && <div className="certificate-modal-backdrop" role="dialog" aria-modal="true" aria-label="Certificate workflow"><section className="certificate-modal certificate-modal--wide"><h2>{batch.renderMode === "template" ? "Print on Template" : "Certificate"} {batch.index + 1} of {batch.ids.length}</h2><p>Student: <strong>{currentStudent.name}</strong></p>{preview ? <iframe id="certificate-preview-frame" title={`Certificate preview for ${currentStudent.name}`} src={preview.url} className="certificate-preview-frame" /> : <p className="admin-muted">Prepare this certificate to preview, print, or download it.</p>}<div className="admin-actions-row">{!preview ? <button className="admin-primary-btn" type="button" disabled={busy} onClick={prepare}>{busy ? "Generating..." : "Preview Certificate"}</button> : <><button className="admin-secondary-btn" type="button" onClick={print}>Print</button><button className="admin-primary-btn" type="button" onClick={download}>Download</button><button className="admin-secondary-btn" type="button" onClick={next}>{batch.index + 1 === batch.ids.length ? "Finish" : "Next Certificate"}</button></>}<button className="admin-secondary-btn" type="button" disabled={busy} onClick={() => { if (preview?.url) URL.revokeObjectURL(preview.url); setPreview(null); setBatch(null); }}>Cancel</button></div></section></div>}
  </main>;
}
