...
import { formatUtilization, getBranchDivisionRecommendations } from "../../utils/administrationAnalytics";
...
export default function StudentDivisionRecommendation({ student }) {
  ...
  return <>
    ...
    {open && <div className="administration-dialog-backdrop recommendation-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}><section className="recommendation-modal" role="dialog" aria-modal="true" aria-labelledby="division-recommendation-title">
      <div className="recommendation-modal__heading"><div><p className="portal-eyebrow">Branch-based recommendation</p><h2 id="division-recommendation-title">Suggest Division</h2><p>Live capacity for <strong>{student.branch}</strong>. Full branches are excluded from recommendations.</p></div><button className="admin-secondary-btn" type="button" onClick={() => setOpen(false)}>Close</button></div>
      {loading ? <div className="analytics-skeleton"><span /><span /><span /></div> : error ? <p className="admin-error">{error}</p> : <div className="recommendation-table-wrap"><table className="recommendation-table"><thead><tr><th>Division</th><th>Utilization %</th><th>Students Allocated</th><th>Available Seats</th></tr></thead><tbody>{recommendations.map((row, index) => <tr className={index === 0 ? "recommendation-table__best" : ""} key={row.division}><td><strong>{row.division}</strong>{index === 0 && <span className="recommendation-badge">Best match</span>}</td><td><span className="recommendation-utilization"><i style={{ width: `${Math.min(100, row.utilization)}%` }} />{formatUtilization(row.utilization)}</span></td><td>{row.allocatedStudents} / {row.configuredSeats}</td><td>{row.availableSeats}</td></tr>)}</tbody></table>{!recommendations.length && <div className="analytics-empty"><p>No divisions with available seats for this branch.</p></div>}</div>}
    </section></div>}
  </>;
}
