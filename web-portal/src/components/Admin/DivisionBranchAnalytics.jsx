...
import { formatUtilization, getAllocatedStudents, getDivisionAllocationRows } from "../../utils/administrationAnalytics";
...
export default function DivisionBranchAnalytics({ administration, students, loading, error }) {
  ...
  const divisionCapacityRows = useMemo(() => getDivisionAllocationRows(administration.divisions, administration.divisionConfigurations, students), [administration.divisions, administration.divisionConfigurations, students]);
  ...
  return <section className="administration-card administration-card--analytics">
    ...
    {loading ? <div className="analytics-skeleton"><span /><span /><span /></div> : error ? <div className="analytics-empty"><span aria-hidden="true">!</span><p>{error}</p></div> : <>
      ...
      {mode === "division" && <div className="analytics-table-wrap"><table className="analytics-table"><thead><tr><th>Division</th><th>Utilization %</th><th>Allocated</th><th>Available</th></tr></thead><tbody>{divisionCapacityRows.map((row) => <tr key={row.division}><td>{row.division}</td><td>{formatUtilization(row.utilization)}</td><td>{row.allocatedStudents} / {row.totalVacancy}</td><td>{row.isUnconfigured ? "NULL" : row.isFull ? "Full" : row.availableSeats}</td></tr>)}</tbody></table></div>}
    </>}
  </section>;
}
